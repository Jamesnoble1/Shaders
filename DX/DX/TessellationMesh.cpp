...
TessellationMesh::TessellationMesh(ID3D11Device* device, WCHAR* textureFilename, WCHAR* displacementFileName)
{
	...
	LoadDisplacement(device, displacementFileName);
}
...
void TessellationMesh::InitBuffers(ID3D11Device* device)
{
	...
	vertices[1].position = XMFLOAT3(0.0f, 1.0f, 0.0f);  // Bottom left.
...
void TessellationMesh::LoadDisplacement(ID3D11Device* device, WCHAR* filename)
{
	// Create the texture object.
	Displacement = new Texture(device, filename);
}

ID3D11ShaderResourceView* TessellationMesh::getDisplacement()
{
	return Displacement->GetTexture();
}